...
void isl_handle_error(__isl_keep isl_ctx *ctx, enum isl_error error, __isl_keep const char *msg,
	__isl_keep const char *file, int line);
...
int isl_ctx_parse_options(__isl_keep isl_ctx *ctx, int argc, char **argv, unsigned flags);
void isl_ctx_ref(__isl_keep struct isl_ctx *ctx);
void isl_ctx_deref(__isl_keep struct isl_ctx *ctx);
void isl_ctx_free(__isl_take isl_ctx *ctx);

void isl_ctx_abort(__isl_keep isl_ctx *ctx);
void isl_ctx_resume(__isl_keep isl_ctx *ctx);
int isl_ctx_aborted(__isl_keep isl_ctx *ctx);

void isl_ctx_set_max_operations(__isl_keep isl_ctx *ctx, unsigned long max_operations);
unsigned long isl_ctx_get_max_operations(__isl_keep isl_ctx *ctx);
void isl_ctx_reset_operations(__isl_keep isl_ctx *ctx);

#define ISL_ARG_CTX_DECL(prefix,st,args)				\
st *isl_ctx_peek_ ## prefix(__isl_keep isl_ctx *ctx);
...
int prefix ## _get_ ## field(__isl_keep isl_ctx *ctx)				\
{									\
	st *options;							\
...
isl_stat prefix ## _set_ ## field(__isl_keep isl_ctx *ctx, int val)		\
{									\
	st *options;							\
	options = isl_ctx_peek_ ## prefix(ctx);				\
	if (!options)							\
...
__isl_keep const char *prefix ## _get_ ## field(__isl_keep isl_ctx *ctx)			\
{									\
	st *options;							\
	options = isl_ctx_peek_ ## prefix(ctx);				\
	if (!options)							\
...
isl_stat prefix ## _set_ ## field(__isl_keep isl_ctx *ctx, __isl_keep const char *val)	\
{									\
	st *options;							\
	options = isl_ctx_peek_ ## prefix(ctx);				\
	if (!options)							\
...
enum isl_error isl_ctx_last_error(__isl_keep isl_ctx *ctx);
__isl_keep const char *isl_ctx_last_error_msg(__isl_keep isl_ctx *ctx);
__isl_keep const char *isl_ctx_last_error_file(__isl_keep isl_ctx *ctx);
int isl_ctx_last_error_line(__isl_keep isl_ctx *ctx);
void isl_ctx_reset_error(__isl_keep isl_ctx *ctx);
void isl_ctx_set_error(__isl_keep isl_ctx *ctx, enum isl_error error);
